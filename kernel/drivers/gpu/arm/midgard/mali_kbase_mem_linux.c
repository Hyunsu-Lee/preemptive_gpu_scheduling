...
struct kbase_va_region *kbase_mem_alloc(kbase_context *kctx, u64 va_pages, u64 commit_pages, u64 extent, u64 *flags, u64 *gpu_va, u16 *va_alignment)
{
	...
	*gpu_va = 0; /* return 0 on failure */
	...
	if(kctx->sched_info.gpgpu_priority > SCHED_RT_PRIORITY){
		...
		if(kctx->ik_ctx_info.is_ik == IK_CTX_F_TRANS)
			init_ikernel_region(reg);

#ifdef _TSK_CUSTOM_PT_
		init_ikernel_pt_info(reg);
#endif


		if (kctx->ik_ctx_info.is_ik <= IK_CTX_PARTAIL
			&& (((reg->flags & KBASE_REG_ZONE_MASK) == KBASE_REG_ZONE_SAME_VA && s==1) ||
...
#ifdef _TSK_CUSTOM_IKERNEL_
	if(kctx->ik_ctx_info.is_ik == IK_CTX_F_TRANS && 
			((reg->flags & KBASE_REG_ZONE_MASK) == KBASE_REG_ZONE_SAME_VA)){
		add_ikernel_region(kctx, reg);
	}
#endif
