			     ...
			     int job_slot_idx)
{
	...
	int base_priority = kctx->sched_info.gpgpu_priority;
			...
			if (!list_empty(job_list)) {
				...
				*katom_ptr = list_entry(job_list->next, kbase_jd_atom, sched_info.cfs.list);
				...
				list_for_each_entry(tmp_ctx, &kctx->jctx.sched_info.runpool.policy_ctx.cfs.list, jctx.sched_info.runpool.policy_ctx.cfs.list){
					...
					if(base_priority < tmp_ctx->sched_info.gpgpu_priority){
...
#ifdef _TSK_CUSTOM_SCHED_
		kctx->sched_info.gpgpu_priority = -1*ctx_info->process_priority-40;
#ifdef _TSK_CUSTOM_SCHED_MFQ_
		kctx->sched_info.mfq.original_priority = kctx->sched_info.gpgpu_priority;
#endif
#endif
	} else {
...
#ifdef _TSK_CUSTOM_SCHED_
		kctx->sched_info.gpgpu_priority = ctx_info->process_priority;
#ifdef _TSK_CUSTOM_SCHED_MFQ_
		kctx->sched_info.mfq.original_priority = kctx->sched_info.gpgpu_priority;
#endif
#endif
...
void kbasep_js_policy_runpool_add_ctx(kbasep_js_policy *js_policy, kbase_context *kctx)
{
	...
	if(list_empty(&policy_info->scheduled_ctxs_head)){
		...
		list_for_each_entry(pos, &policy_info->scheduled_ctxs_head, jctx.sched_info.runpool.policy_ctx.cfs.list){
			...
			if(pos->sched_info.gpgpu_priority > kctx->sched_info.gpgpu_priority){
				list_add(&kctx->jctx.sched_info.runpool.policy_ctx.cfs.list,
	...
	//printk(KERN_ALERT"Add CTX:%u (%d, %d)\n", kctx->tInfo.ctx_id, kctx->sched_info.gpgpu_priority, kctx->sched_info.mfq.original_priority);
#else
	list_add_tail(&kctx->jctx.sched_info.runpool.policy_ctx.cfs.list, &policy_info->scheduled_ctxs_head);
#endif
...
void kbasep_js_policy_runpool_remove_ctx(kbasep_js_policy *js_policy, kbase_context *kctx)
{
	...
	list_del(&kctx->jctx.sched_info.runpool.policy_ctx.cfs.list);
	//printk(KERN_ALERT"Remove CTX:%u (%d, %d)\n", kctx->tInfo.ctx_id, kctx->sched_info.gpgpu_priority, kctx->sched_info.mfq.original_priority);
}
...
#ifdef _TSK_CUSTOM_SCHED_
			/*
			 * nr_dep_job 은 IOCTL을 통해 요청된 작업이 개별적으로 요청되는 것이 아닌
			 * 여러개가 동시에 묶여서 들어온 작업임.
			 * 여기 스케줄링 타이밍은 외부 요청과 내부 드라이버 동작에서 호출되기 때문에
			 * 실제로 높은 우선순위의 kctx가 작업 정리과정에 있어서 당장은 run_queue에는
			 * 작업이 없지만, 바로 수행이 되려고 하는 타이밍인데, 외부에서 요청된 스케줄링으로
			 * 해당 작업이 꼬여버릴 수도 있음.
			 * 그래서 nr_prev_dep_job 즉, 현재 선택된 kctx보다 높은 우선순위의 prev_kctx에
			 * 디펜던시가 있는 작업이 있을 경우(혹은 해당 kctx가 선점된 작업)이면
			 * 스케줄러는 선택하지 않음. 어차피 enqueue 될 것이라서 스케줄링 하지 말자는 의미.
			 * 그러니 mfq에서는 우선순위를 따로 바꾸어도 괜찮을 것 같음.
			 */
			if(prev_kctx != NULL)
				if(kctx->sched_info.gpgpu_priority >
						prev_kctx->sched_info.gpgpu_priority){
...
void kbasep_js_policy_enqueue_job(kbasep_js_policy *js_policy, kbase_jd_atom *katom)
{
	...
	if(katom->sched_stat & (SCHED_STAT_PSLOT | SCHED_STAT_PSNAP)){
		...
		*(klc_info+(katom->jc_indirect&0xfff)/4) = 0x0;
		...
		if(katom->ik_arg_info.is_ik == IK_ATOM_PT)
	...
	if(list_empty(&ctx_info->job_list_head[job_info->cached_variant_idx])){
		...
		if(!(katom->sched_stat & SCHED_STAT_PRUN)){
			parent_ctx->sched_info.mfq.delay_start = ktime_get();
		}
