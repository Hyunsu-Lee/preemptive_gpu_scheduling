...
void kbase_job_submit_nolock(kbase_device *kbdev, kbase_jd_atom *katom, int js)
{
	...
	if(jm_slots[js].submitted[jm_slots[js].submitted_head & 15] == katom){
		...
		if(katom->ik_arg_info.is_ik || katom->kctx->sched_info.gpgpu_priority == SCHED_RT_PRIORITY){
#else
		if(katom->kctx->sched_info.gpgpu_priority == SCHED_RT_PRIORITY){
#endif
...
#endif
#ifdef _TSK_CUSTOM_IKERNEL_
	if(katom->kctx->ik_ctx_info.is_ik == IK_CTX_F_TRANS){
		ikernel_classify_param(katom);
	}
#endif
...
#ifdef _TSK_CUSTOM_IKERNEL_                                                                                                       
		        if(head_slot_atom->kctx->ik_ctx_info.is_ik ||
				head_slot_atom->kctx->sched_info.gpgpu_priority == SCHED_RT_PRIORITY){
				
				if(head_slot_atom->ik_arg_info.is_ik == IK_ATOM_TRANS){
					if(ikernel_is_dep(katom, head_slot_atom)){
						head_slot_atom->sched_stat |= SCHED_STAT_ROP_WAIT;
						katom->ik_arg_info.rop_dep_atom = head_slot_atom;
...
#else
			if(head_slot_atom->kctx->sched_info.gpgpu_priority == SCHED_RT_PRIORITY){
				head_slot_atom->sched_stat |= SCHED_STAT_RUN;
				sc_hw_submit(kbdev, head_slot_atom, 1);
#endif
			}else{
