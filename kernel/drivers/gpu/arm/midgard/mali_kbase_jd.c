...
#endif
//#ifdef _TSK_CUSTOM_SNAP_ || _TSK_CUSTOM_SCHED_
#if defined(_TSK_CUSTOM_SNAP_) && defined(_TSK_CUSTOM_SCHED_)
	katom->nr_param = 0;
	INIT_LIST_HEAD(&katom->param_list);

	//rt_skip
	if(kctx->sched_info.gpgpu_priority > SCHED_RT_PRIORITY
			&& !(katom->core_req & BASE_JD_REQ_SOFT_JOB)){
#ifdef _TSK_CUSTOM_IKERNEL_
	if(!ikernel_submit_atom(katom)){
#endif // IKERN
		init_snapshot_info(katom);
#ifdef _TSK_CUSTOM_IKERNEL_
	}
#endif // IKERN
	}
...
#ifdef _TSK_CUSTOM_IKERNEL_
	if(katom->ik_arg_info.is_ik == IK_ATOM_TRANS){
		ikernel_restore(katom);
		if(katom->ik_arg_info.rop_dep_atom->sched_stat & (SCHED_STAT_SLOT_READY | SCHED_STAT_ROP_WAIT)){
			katom->ik_arg_info.rop_dep_atom->sched_stat &= ~SCHED_STAT_ROP_WAIT;
			katom->ik_arg_info.rop_dep_atom->sched_stat |= SCHED_STAT_RUN;
			sc_hw_submit(kbdev, katom->ik_arg_info.rop_dep_atom, 1);
		}
	}
	//if(kctx->ik_ctx_info.is_ik)
	//	ikernel_restore(katom);
#endif
