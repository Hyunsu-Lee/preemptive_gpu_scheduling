...
void kbase_free_alloced_region(struct kbase_va_region *reg)
{
	...
	if(reg->kctx->ik_ctx_info.is_ik == IK_CTX_F_TRANS){
		if((reg->flags & KBASE_REG_ZONE_MASK) == KBASE_REG_ZONE_SAME_VA && s==1){
			release_ikernel_region(reg);
		}else if(((reg->flags & KBASE_REG_ZONE_MASK) == KBASE_REG_ZONE_CUSTOM_VA) && s>=64){
			release_ikernel_buffer_map(reg);
		}
	//ik 였다면, 해당 region이 list에 있는지 확인하고, vmap 해제한다.
	//region에도 ik 정보를 담고 있다면, list search 과정을 줄일 수 있음
	}
#ifdef _TSK_CUSTOM_PT_
        release_ikernel_pt_info(reg);
#endif	
	if(reg->kctx->ik_ctx_info.is_ik <= IK_CTX_PARTAIL
			&& reg->kctx->sched_info.gpgpu_priority > SCHED_RT_PRIORITY){
#else
	if(reg->kctx->sched_info.gpgpu_priority > SCHED_RT_PRIORITY){
#endif
