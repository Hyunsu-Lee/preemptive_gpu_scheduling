...
kbase_context *kbase_create_context(kbase_device *kbdev)
{
	...
	init_ikernel_ctx(kctx);
#endif
#ifdef _TSK_CUSTOM_SNAP_
	spin_lock_init(&kctx->snap_lock);
#endif
#if defined(_TSK_CUSTOM_PROC_) || defined(_TSK_CUSTOM_IKERNEL_)
...
void kbase_destroy_context(kbase_context *kctx)
{
	...
	//IKERNEL CTX Management List
	delete_ikernel_ctx(kctx->ik_ctx_info.fd);
	//if(delete_ikernel_ctx(kctx->ik_ctx_info.fd))
	//	printk(KERN_ALERT"MALI ERROR: ikernel destroy\n");
#endif
