...
float *OpenClFindNearestNeighbors(
    ...
    cl_kernel NN_kernel;
        ...
        cl_NN_program = cl_compileProgram(
            (char *)"nearestNeighbor_kernel.cl","-I./");
    ...
    //IKERN PT
    size_t localWorkSize[1] = {PT_WG_SIZE};
    INIT_PT_WORK_ITEM()
    SET_PT_WORK_ITEM(globalWorkSize, localWorkSize)
    SET_PT_ARG(NN_kernel, 5)


    error = clEnqueueNDRangeKernel(
        command_queue,  NN_kernel, PT_WORK_DIM, 0,
        PT_GWORK, PT_LWORK,
        0, NULL, &kernelEvent);
