...
int main(int argc, char** argv)
{
  ...
  //IKERN_PT
  INIT_PT_VARIABLE()
  CREATE_PT_BUFFER(context, commands)
  INIT_PT_WORK_ITEM()
  ...
  for(j = 0; j < iterations; j++)
    {
      ...
      local[1] = WG_SIZE_Y;
      //IKERN_PT
      SET_PT_WORK_ITEM(global, local)
      SET_PT_ARG(ko_vadd, 14)
      printf("->>kid[%d]: g[%zu], l[%zu]\n", j, pt_gwork[0], pt_lwork[0]);
      err = clEnqueueNDRangeKernel(commands, ko_vadd, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
      PT_BUFFER_DEBUG(commands)
