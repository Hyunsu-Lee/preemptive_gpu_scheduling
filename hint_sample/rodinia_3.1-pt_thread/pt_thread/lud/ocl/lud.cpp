...
main ( int argc, char *argv[] )
{
  printf("WG size of kernel = %d X %d\n", BLOCK_SIZE, BLOCK_SIZE);
	...
	sprintf(clOptions," ");
#ifdef BLOCK_SIZE
	sprintf(clOptions + strlen(clOptions), "-I./ -DBLOCK_SIZE=%d", BLOCK_SIZE);
#endif
    ...
    INIT_PT_VARIABLE()
    INIT_PT_WORK_ITEM()
    CREATE_PT_BUFFER(context, cmd_queue)
	...
	for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
      ...
      //IKERN PT
      SET_PT_WORK_ITEM(global_work1, local_work1)
      SET_PT_ARG(diagnal, 4)
	  err = clEnqueueNDRangeKernel(cmd_queue, diagnal, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
      PT_BUFFER_DEBUG(cmd_queue)
      ...
      //IKERN PT
      SET_PT_WORK_ITEM(global_work2, local_work2)
      SET_PT_ARG(perimeter, 6)
	  err = clEnqueueNDRangeKernel(cmd_queue, perimeter, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
      PT_BUFFER_DEBUG(cmd_queue)
      ...
      //IKERN PT
      SET_PT_WORK_ITEM(global_work3, local_work3)
      SET_PT_ARG(internal, 5)
	  err = clEnqueueNDRangeKernel(cmd_queue, internal, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
      PT_BUFFER_DEBUG(cmd_queue)
