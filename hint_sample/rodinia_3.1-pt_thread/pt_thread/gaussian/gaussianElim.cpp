...
void ForwardSub(cl_context context, float *a, float *b, float *m, int size,int timing){    
    ...
    gaussianElim_program = cl_compileProgram(
        (char *)"gaussianElim_kernels.cl","-I./");
    ...
    if(status)exit(1);
    //IKERNEL
    ikernel_set_command(IK_ATOM_PT);
    ...
    //printf("(%zu, %f) (%zu, %f) (%zu, %f)\n", sizeof(float)*size*size, *a, sizeof(float)*size, *b, sizeof(float)*size*size);

    cl_command_queue command_queue = cl_getCommandQueue();

    //IKERN_PT
    INIT_PT_VARIABLE()
    CREATE_PT_BUFFER(context, command_queue)
    INIT_PT_WORK_ITEM()
...
izeFan2Buf[1])*localWorksizeFan2Buf[1];
	...
	for (t=0; t<(size-1); t++) {
        ...
        //IKERN_PT
        if(localWorksizeFan1 == NULL)
            printf("test1\n");
        if(localWorksizeFan2 == NULL)
            printf("test1\n");
        SET_PT_WORK_ITEM(globalWorksizeFan1, localWorksizeFan1)
        SET_PT_ARG(fan1_kernel, 5)
        printf("->>Fan1:kid[%d]: g[%zu], l[%zu]\n", t, pt_gwork[0], pt_lwork[0]);
        ...
        // launch kernel
        //IKERN_PT
        error = clEnqueueNDRangeKernel(command_queue, fan1_kernel, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, &kernelEvent);
        PT_BUFFER_DEBUG(command_queue)
...
//             printf("here1b\n");
        ...
        //IKERN_PT
        SET_PT_WORK_ITEM(globalWorksizeFan2, localWorksizeFan2)
        SET_PT_ARG(fan2_kernel, 5)
        printf("->>Fan2:kid[%d]: g[%zu], l[%zu]\n", t, pt_gwork[0], pt_lwork[0]);

        cl_errChk(argchk,"ERROR in Setting Fan2 kernel args",true);
        
        // launch kernel
        //IKERN_PT
        error = clEnqueueNDRangeKernel(command_queue, fan2_kernel, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, &kernelEvent);
        PT_BUFFER_DEBUG(command_queue)
