...
#define CREATE_PT_BUFFER(context, command) \
    if(!pt_info){ \
        ...
        for(ik_pt_bi=0;ik_pt_bi<NR_SHADER_CORE;ik_pt_bi++) \
            for(ik_pt_bj=0;ik_pt_bj<PT_INFO_SIZE;ik_pt_bj++) \
                pt_info_p[ik_pt_bi*NR_SHADER_CORE+ik_pt_bj] = 0; \
...
#define SET_PT_WORK_ITEM(gvar, lvar) \
    ...
    for(ik_pt_wi=0;ik_pt_wi<gdimension;ik_pt_wi++){ \
        nr_pt *= *(gvar+ik_pt_wi); \
        pt_work_info[0][ik_pt_wi] = *(gvar+ik_pt_wi); \
    } \
    if(lvar!=NULL){ \
    ldimension = sizeof(lvar)/sizeof(size_t); \
    for(ik_pt_wi=0;ik_pt_wi<ldimension;ik_pt_wi++){ \
        nr_lpt *= *(lvar+ik_pt_wi); \
        pt_work_info[1][ik_pt_wi] = *(lvar+ik_pt_wi); \
    } \
    }else{ \
        printf("testtest\n"); \
        nr_lpt = PT_WG_SIZE; \
        pt_work_info[1][0] = PT_WG_SIZE; \
    } \
    pt_gwork[0] = nr_pt; \
    pt_lwork[0] = nr_lpt; \
    if(nr_lpt>PT_WG_SIZE) \
        pt_lwork[0] = PT_WG_SIZE; \
    if(nr_pt>PT_TOTAL_SIZE_LIMIT) \
