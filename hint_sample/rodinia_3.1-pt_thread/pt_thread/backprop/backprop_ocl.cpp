...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
    ...
    /*
    for(int t=0;t<(in + 1) * sizeof(float);t++)
        printf("%d ", net->input_units);
    printf("\n");
    for(int t=0;t<(in + 1) * (hid + 1) * sizeof(float);t++)
        printf("%d ", input_weights_one_dim);
    printf("\n");
    */
