...
#define MAX_THREADS_PER_BLOCK PT_WG_SIZE
...
void run_bfs_gpu(int no_of_nodes, Node *h_graph_nodes, int edge_list_size, \
	...
	try{
        ...
        INIT_PT_VARIABLE()
        INIT_PT_WORK_ITEM()
        CREATE_PT_BUFFER(oclHandles.context, oclHandles.queue)
		...
		do{
            ...
            size_t global_work[3] = {no_of_nodes, 1, 1};
            size_t local_work[3] = {work_group_size, 1, 1};
            if(global_work[0]%local_work[0] != 0)
                global_work[0] = global_work[0] + (local_work[0]-(global_work[0]%local_work[0]));
            
            //IKERN PT
            SET_PT_WORK_ITEM(global_work, local_work)
            SET_PT_ARG(oclHandles.kernel[kernel_id], kernel_idx)
			_clInvokeKernel(kernel_id, PT_GWORK[0], PT_LWORK[0]);
