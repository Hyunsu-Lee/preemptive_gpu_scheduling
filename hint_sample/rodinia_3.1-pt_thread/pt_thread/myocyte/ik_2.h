...
#define CREATE_PT_BUFFER(context, command) \
    ...
    if(!pt_info){ \
        ...
        for(i1=0;i1<NR_SHADER_CORE;i1++) \
            for(j1=0;j1<PT_INFO_SIZE;j1++) \
                pt_info_p[i1*NR_SHADER_CORE+j1] = 0; \
...
#define SET_PT_WORK_ITEM(gvar, lvar) \
    ...
    for(ii=0;ii<gdimension;ii++){ \
        nr_pt *= *(gvar+ii); \
        pt_work_info[0][ii] = *(gvar+ii); \
    } \
    ldimension = sizeof(lvar)/sizeof(size_t); \
    for(ii=0;ii<ldimension;ii++){ \
        nr_lpt *= *(lvar+ii); \
        pt_work_info[1][i] = *(lvar+ii); \
    }\
    pt_gwork[0] = nr_pt; \
    pt_lwork[0] = nr_lpt; \
    if(nr_pt>PT_TOTAL_SIZE_LIMIT) \
