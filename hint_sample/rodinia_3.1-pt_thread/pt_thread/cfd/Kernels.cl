...
------------------------------------------------------------*/
__kernel void memset_kernel(__global char * mem_d, short val, int ct, IKERN_PT_PARAM()){
    INIT_PT_IKERN()
    PT_IKERN_START(){

	const int thread_id = PT_GT_ID;//get_global_id(0);
...
__kernel void initialize_variables(__global float* variables, __constant float* ff_variable, int nelr, IKERN_PT_PARAM()){
    
    INIT_PT_IKERN()
    PT_IKERN_START(){

	//const int i = (blockDim.x*blockIdx.x + threadIdx.x);
	const int i = PT_GT_ID;//get_global_id(0);
	if( i >= nelr) return;
...
__kernel void time_step(int j, int nelr, 
				...
				__global float* variables, 
				__global float* step_factors, 
				__global float* fluxes, IKERN_PT_PARAM()){
    INIT_PT_IKERN()
    PT_IKERN_START(){
	//const int i = (blockDim.x*blockIdx.x + threadIdx.x);
	const int i = PT_GT_ID;//get_global_id(0);
