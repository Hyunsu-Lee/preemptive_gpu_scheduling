...
------------------------------------------------------------*/
//IKERN PT
extern cl_int *pt_info_p;
extern cl_mem pt_info;

void _clInvokeKernel(int kernel_id, int work_items, int work_group_size, int arg_idx) throw(string){
#ifdef PROFILE_
	double t1 = gettime();
#endif
    ...
    //IKERN PT 
    //printf("kid[%d]: g[%zu], l[%zu]\n", kernel_id, global_work_size[0], local_work_size[0]);
    INIT_PT_WORK_ITEM()
    SET_PT_WORK_ITEM(global_work_size, local_work_size)
    SET_PT_ARG(oclHandles.kernel[kernel_id], arg_idx)
    printf("->>kid[%d]: g[%zu], l[%zu]\n", kernel_id, pt_gwork[0], pt_lwork[0]);
	oclHandles.cl_status = clEnqueueNDRangeKernel(oclHandles.queue, oclHandles.kernel[kernel_id], PT_WORK_DIM, 0, \
											PT_GWORK, PT_LWORK, 0 , 0, &(e[0]) );	
...
#endif
    //IKERN PT
    PT_BUFFER_DEBUG(oclHandles.queue)
}
...
void _clMemset(cl_mem mem_d, short val, int number_bytes)throw(string){
    ...
    /IKERN PT
    size_t gwork[3] = {(size_t)work_items,1, 1};
    size_t lwork[3] = {(size_t)work_group_size,1, 1};
    SET_PT_WORK_ITEM(gwork, lwork)
