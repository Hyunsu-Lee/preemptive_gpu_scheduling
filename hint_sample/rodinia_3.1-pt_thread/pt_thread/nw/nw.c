...
int main(int argc, char **argv){
  ...
  printf("WG size of kernel = %d \n", BLOCK_SIZE);
	...
	clSetKernelArg(kernel2, 3, sizeof(cl_int) * (BLOCK_SIZE + 2) *(BLOCK_SIZE+2), (void*)NULL );
	...
	for( int blk = 1 ; blk <= worksize/BLOCK_SIZE ; blk++){
		...
		//IKERN PT
		SET_PT_WORK_ITEM(global_work, local_work)
		SET_PT_ARG(kernel1, 12)

		//printf("BLOCK_SIZE: %d, %d, %d\n", BLOCK_SIZE, blk, BLOCK_SIZE*blk);
		clSetKernelArg(kernel1, 7, sizeof(cl_int), (void*) &blk);
		//printf("lwsize : %zu\n", lwsize);


		//IKERN PT
		err = clEnqueueNDRangeKernel(cmd_queue, kernel1, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
	...
	for( int blk =  worksize/BLOCK_SIZE - 1  ; blk >= 1 ; blk--){	   
		...
		//IKERN PT
		SET_PT_WORK_ITEM(global_work, local_work)
		SET_PT_ARG(kernel2, 12)

		//IKERN PT
		err = clEnqueueNDRangeKernel(cmd_queue, kernel2, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
...
/*	ae = gettime();
	...
	mem_size = (max_cols * max_rows * sizeof(int))*3/1024/1024;
	printf("mem_size : %lf\n", mem_size);
*/
