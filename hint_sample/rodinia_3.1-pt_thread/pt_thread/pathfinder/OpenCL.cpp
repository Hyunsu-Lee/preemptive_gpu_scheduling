...
void OpenCL::launch(string toLaunch, cl_mem pt_info, int idx)
{
    //IKERN PT
    INIT_PT_WORK_ITEM()
    SET_PT_WORK_ITEM(&gwsize, &lwsize)
    SET_PT_ARG(kernelArray[toLaunch], idx)
...
void OpenCL::buildKernel()
{
	...
	ret = clBuildProgram(program, NULL, NULL, "-I./", NULL, NULL);
