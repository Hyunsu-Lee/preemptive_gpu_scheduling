...
int allocate(int n_points, int n_features, int n_clusters, float **feature)
{
    ...
    //IKERNEL PT
    INIT_PT_WORK_ITEM()
    SET_PT_WORK_ITEM(global_work, &local_work_size)
    SET_PT_ARG(kernel2, 4)
           ...
           float  **new_centers)	
{
    ...
    INIT_PT_WORK_ITEM()
    SET_PT_WORK_ITEM(global_work, &local_work_size)
    SET_PT_ARG(kernel_s, 8)

	err = clEnqueueNDRangeKernel(cmd_queue, kernel_s, PT_WORK_DIM, NULL, PT_GWORK, PT_LWORK, 0, 0, 0);
    PT_BUFFER_DEBUG(cmd_queue)
